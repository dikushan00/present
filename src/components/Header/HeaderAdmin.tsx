    ...
    return <div className={`${styles.header__auth__block__item} ${styles.header__auth}`} ref={dropdownRef}>
        <div onClick={() => setIsShowDropdown(prevState => !prevState)} className={styles.header__auth__block}>
            ...
            <div className={styles.avatar}>
                ...
                <i className={`fas fa-sort-down ${styles.auth__arrow} ${isShowDropdown ? styles.active : ""}`} aria-hidden/>
            </div>
        </div>
        {
...
const NotAuthUserBlock = () => {
    ...
    return <div className={`${styles.header__auth} flex`}>
        <Button onClick={() => openModal("modal-login")} type="primary" className={`btn ${styles.header_auth__btn}`}>Войти</Button>
        <Button onClick={() => openModal("modal-sign")} className={`btn ${styles.header_auth__btn}`}>Регистрация</Button>
    </div>
}
